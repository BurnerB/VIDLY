...
router.get('/:id',async (req,res)=>{
    ...
    if(!customers) return res.status(404).send("The Customer with the ID was not found.")
    res.send(customers);
})
...
router.put('/:id', auth, async(req, res)=>{
    ...
    if(!customer) return res.status(404).send("The Customer with the ID was not found.")
